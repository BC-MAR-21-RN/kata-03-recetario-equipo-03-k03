...
const MealDets = ({ navigation, route }) => {
    const { mealName, mealIngredients, mealCategory, imgUrl, mealServings, mealFav } = route.params;
    ...
    return (
        <View style={styles.mealDets}>
            ...
            <View style={styles.iconBar}>
                <Icon name="times" size={30} color="white" style={styles.closeIcon} onPress={() => navigation.goBack()} />
                <Icon name="share-alt" size={25} color="white" style={styles.singleIcon}/>
                {isFav == true ?
                    <Icon name='heart' size={25} color="white" style={styles.singleIcon} onPress={() => changeFavStatus(mealName)} /> :
                    <Icon name='heart-o' size={25} color="white" style={styles.singleIcon} onPress={() => changeFavStatus(mealName)} />}
            </View>
            <Text style={styles.categoryText}>{mealCategory}</Text>
            <Text style={styles.titleText}>{mealName}</Text>

            <ScrollView >
                <Text style={styles.servingsText}>Ingredients{"\n"}for {mealServings} servings</Text>
                {
